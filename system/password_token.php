...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (!empty($Email)) {
        ...
        $sql = "SELECT * FROM tbl_users WHERE Email='$Email' AND Status = 1 ";
        $result = $db->query($sql);
        $result->num_rows;
        if ($result->num_rows <= 0) {
            ...
            $resetmemberid = $row['UserId'];
            $_SESSION['resetmemberidz'] = $row['UserId'];
            $resetTitle = $row['Title'];

            $resetFirst_Name = $row['FirstName'];
            $resetLast_Name = $row['LastName'];
            $resetEmail = $row['Email'];

            //creating sessions to use resetpasword2.php

            $_SESSION['resetFirst_Name'] = $row['FirstName'];
            $_SESSION['resetLast_Name'] = $row['LastName'];
...
//                              echo $_SESSION['passwordtoken'];
                $db = dbConn();
                $sql = " UPDATE tbl_users SET passwordreset='$memberpasswordreset' WHERE UserId='$resetmemberid' ";
                $result = $db->query($sql);
    ...
    <section id="login" class="login">
        <div class="container" data-aos="fade-up">
            ...
            <div class="section-header">
                ...
                <p>To get back with us, please enter your email and proceed...</p>
            </div>
