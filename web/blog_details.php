...
<!--                        <div class="meta-bottom">
                    ...
                    <div class="comments">
                        ...
                        <h4 class="comments-count">
                            <?php
                            $sql = "SELECT count(*) FROM tbl_blog_comments WHERE comment_post_id=$blogid AND comment_status=1 ";
                            $db = dbConn();
                            $result = $db->query($sql);
                        ...
                        <div id="comment-1" class="comment">
                            <?php
                            $db = dbConn();
                            $sql = "SELECT * FROM tbl_blog_comments WHERE comment_post_id=$blogid AND comment_status=1";
                            $result = $db->query($sql);
                            ?>
        ...
        <!--                                        <div class="comment-img"><img src="assets/img/blog/comments-1.jpg" alt=""></div>-->
                                        <div>
                                            <h5><?= $row['comment_author'] ?></h5>
                                            <time datetime="2020-01-01"><?= $row['comment_date'] ?></time>
                                            <p>
                                                <?= $row['comment_content'] ?>
                                            </p>
                                        </div>
                        ...
                        <div class="reply-form">
                            ...
                            if ($_SERVER['REQUEST_METHOD'] == "POST" && @$action == "comment") {
                                ...
                                if (empty($messages)) {
                                    ...
                                    $cdate = date('Y-m-d');
                                     $sql = "INSERT INTO tbl_blog_comments(comment_post_id,comment_author,comment_email,comment_content,comment_date) VALUES ('$blogid','$Name','$Email','$Comment','$cdate')";

                                    $db = dbConn();
                                    $db->query($sql);

                                    $sql2 = "UPDATE tbl_blog SET post_comment_count = post_comment_count + 1 WHERE Id = $blogid";
                                    $db = dbConn();
                                    $db->query($sql2);
...
<!--                                <input type="text" name="Id" value="<?= $Id ?>">-->
                                <input type="hidden" name="blogid" value= "<?= $blogidforcomment ?>">
